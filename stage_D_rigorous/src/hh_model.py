...
class HHModel:
    def __init__(self, config, trial_generators: Dict, seeds_tuple: Tuple = (0,0,0,0)):
        ...
        self.W_in = (self.rng_inmask.random(self.cfg.hh.N) < self.cfg.hh.in_density).astype(float)[:, np.newaxis]
    ...
    def simulate(self, rho: float, bias: float, spikes_in: np.ndarray, 
        ...
        n_steps, n_in = spikes_in.shape
        
        # State Initialization
        if full_state is not None:
             V = full_state['V'].copy()
             m, h, n = full_state['m'].copy(), full_state['h'].copy(), full_state['n'].copy()
             b_gate = full_state['b_gate'].copy()
             s_trace = full_state['s_trace'].copy()
             s_in_trace = full_state['s_in_trace'].copy() # should be (n_in,)
        else:
             V = np.full(N, -65.0)
             m, h, n = self.get_steady_state(V)
             b_gate = 1.0 / (1.0 + np.exp((V + 80.0) / 6.0))
             s_trace = np.zeros(N)
             s_in_trace = np.zeros(n_in) 
        # W_in needs to be (N, n_in) for matrix multiplication.
        # If self.W_in is (N,), it means a single input channel broadcast to all neurons.
        # We need to adapt W_in_eff based on n_in.
        if self.W_in.ndim == 1 and n_in == 1:
            W_in_eff = self.W_in[:, np.newaxis] # Make it (N, 1)
        elif self.W_in.shape == (N, n_in):
            W_in_eff = self.W_in
        else:
            raise ValueError(f"self.W_in shape {self.W_in.shape} is incompatible with spikes_in input dimension {n_in}")

        decay_in = np.exp(-dt / p.tau_in)
        decay_syn = np.exp(-dt / p.tau_syn)
        
        res_spikes = np.zeros((n_steps, N), dtype=np.bool_)
        ...
        v_trace = np.zeros(n_steps) # To be used for Lyapunov
        I_exc_total, I_inh_total = 0.0, 0.0  # E/I balance
        
        for t in range(n_steps):
            V_old = V.copy()
            # Dynamic input trace integration
            s_in_trace = s_in_trace * decay_in + spikes_in[t]
            # Spatial distribution of input current
            I_pulse = (self.cfg.hh.in_gain * (W_in_eff @ s_in_trace)) + bias
